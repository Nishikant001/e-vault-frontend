...
const NAV_AI = getAIAssistantNavItems("Uploader").map(item => ({ key: item.key, label: item.label, icon: "aiAssistant" }));
    ...
    <>
      ...
      <aside className={`fixed top-0 left-0 h-full z-40 flex flex-col w-[252px] bg-[#1A2433] transition-transform duration-300 ${mobileOpen ? "translate-x-0" : "-translate-x-full"} lg:static lg:translate-x-0 lg:flex-shrink-0`}>
        ...
        <div className="flex items-center gap-[10px] px-[18px] py-[14px] border-b border-white/[0.08] bg-white/[0.015]">
          <div className="w-9 h-9 rounded-full bg-gradient-to-br from-purple-500 to-purple-700 flex items-center justify-center text-[11px] font-bold text-white flex-shrink-0 ring-2 ring-white/10">
            UP
          </div>
          <div>
            <div className="text-white/90 text-[12px] font-semibold">{user?.u || "uploader"}</div>
            <div className="text-white/40 text-[10.5px] font-medium mt-px">Uploader</div>
          </div>
        </div>
        ...
        <div className="border-t border-white/[0.08] p-2.5">
          <button
            onClick={onLogout}
            className="w-full flex items-center gap-[9px] px-3.5 py-[10px] text-[13px] text-white/55 hover:text-white/90 hover:bg-white/[0.05] rounded-[9px] transition-all"
          >
            {ICONS.logout} Logout
          </button>
        </div>
...
function Topbar({ title, onMenuClick, onLogout }) {
  ...
  return (
    <header className="h-[60px] bg-white dark:bg-[#1A2433] border-b border-slate-200 dark:border-slate-700 flex items-center px-5 gap-4 flex-shrink-0 sticky top-0 z-20 transition-colors">
      ...
      <span className="bg-purple-50 dark:bg-purple-900/30 text-purple-700 dark:text-purple-400 text-[11px] font-bold px-[12px] py-[5px] rounded-full hidden sm:inline tracking-[0.2px]">
        Uploader
      </span>
      <div className="flex items-center gap-2">
       <NotificationBell />
        <button
          onClick={() => setDark(v => !v)}
          className="w-9 h-9 flex items-center justify-center rounded-[10px] border border-slate-200 dark:border-slate-700 bg-white dark:bg-[#232F40] text-slate-500 dark:text-slate-400 hover:border-slate-300 dark:hover:border-slate-600 transition-colors"
        >
          {dark ? ICONS.sun : ICONS.moon}
        </button>
        <button
          onClick={onLogout}
          className="hidden sm:flex items-center gap-[7px] px-4 py-[8px] rounded-[9px] bg-blue-600 hover:bg-blue-700 text-white text-[11.5px] font-bold transition-colors"
        >
