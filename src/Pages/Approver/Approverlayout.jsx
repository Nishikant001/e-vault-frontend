...
// NOTE: "approvals" no longer has a hardcoded badge — its badge is now
// injected live from the real pending-approvals count (see Sidebar below).
const NAV_ITEMS = [
  { key: "dashboard", label: "Dashboard",        icon: "dashboard" },
  { key: "approvals", label: "Pending Approvals",icon: "approvals" },
...
const NAV_AI = getAIAssistantNavItems("Approver").map(item => ({ key: item.key, label: item.label, icon: "aiAssistant" }));
    ...
    <>
      ...
      <aside className={`fixed top-0 left-0 h-full z-40 flex flex-col w-[252px] bg-[#1A2433] transition-transform duration-300 ${mobileOpen ? "translate-x-0" : "-translate-x-full"} lg:static lg:translate-x-0 lg:flex-shrink-0`}>
        ...
        <div className="flex items-center gap-[10px] px-[18px] py-[14px] border-b border-white/[0.08] bg-white/[0.015]">
          <div className="w-9 h-9 rounded-full bg-gradient-to-br from-orange-500 to-orange-600 flex items-center justify-center text-[11px] font-bold text-white flex-shrink-0 ring-2 ring-white/10">
            AP
          </div>
          <div>
            <div className="text-white/90 text-[12px] font-semibold">{user?.u || "approver"}</div>
            <div className="text-white/40 text-[10.5px] font-medium mt-px">Approver</div>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto py-3 space-y-[2px]">
          ...
          {visible_NAV_ITEMS.map((item) => (
            <NavItem
              ...
              badge={item.key === "approvals" ? pendingApprovalsCount : item.badge}
...
function Topbar({ title, onMenuClick, onLogout, pendingApprovalsCount }) {
  ...
  return (
    <header className="h-[60px] bg-white dark:bg-[#1A2433] border-b border-slate-200 dark:border-slate-700 flex items-center px-5 gap-4 flex-shrink-0 sticky top-0 z-20 transition-colors">
      ...
      <span className="bg-green-50 dark:bg-green-900/30 text-green-700 dark:text-green-400 text-[11px] font-bold px-[12px] py-[5px] rounded-full hidden sm:inline tracking-[0.2px]">
        Approver
      </span>
...
export function ApproverLayout({ activePage, onNavigate, onLogout, title, children, user }) {
  ...
  // Live pending-approvals count: fetched once on mount and then polled,
  // so the sidebar/topbar badge always reflects real data instead of a
  ...
  useEffect(() => {
    ...
    const fetchPendingCount = async () => {
      try {
        const res = await ApprovalActionApi.pending();
        if (cancelled) return;
