...
import { get, decodeToken, getCurrentTenantId, API_BASE_URL } from "../../services/apiClient";
...
// Only these roles can own/manage a department's incoming document
// requests (select a source document, prepare/mask/send a share).
// Everyone else never gets the "Manage Requests" tab at all — the
// masking flow underneath it is only reachable by someone authorized.
...
function RequestsTable({ requests, loading, mode, onManage, openManageId, onManageUpdated }) {
  if (loading) {
          ...
          {requests.map((r) => (
            <React.Fragment key={r.id}>
              <tr className="border-t border-gray-100 dark:border-gray-800">
                ...
                {mode === "shared" && (
                  <td className="px-4 py-3">
                    {r.status === "SHARED" && r.shareId ? (
                      <div className="flex gap-2">
                        <a
                          href={`${API_BASE_URL}${getTemporaryShareViewUrl(r.shareId)}`}
                          target="_blank"
                          rel="noreferrer"
                        ...
                        <a
                          href={`${API_BASE_URL}${getTemporaryShareDownloadUrl(r.shareId)}`}
                          className="rounded-lg bg-blue-600 px-3 py-1.5 text-xs font-medium text-white hover:bg-blue-700"
                        >
                          Download
                        </a>
...
export default function CrossDepartmentAccessPage() {
  ...
  const loadRequests = useCallback(async () => {
    setLoading(true);
    try {
      const res = await getDocumentAccessRequests({ scope: "mine" });
      const list = res?.data || res || [];
  ...
  return (
    <div className="w-full">
      ...
      {activeTab === "shared" && (
        <RequestsTable requests={sharedWithMe} loading={loading} mode="shared" currentUserId={currentUserId} />
      )}
    </div>
  );
}